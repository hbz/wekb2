...
class AjaxHtmlController {
    ...
    def addToCollection() {
        ...
        if (domain_class) {
                ...
                if (editable || contextObj.id == user.id) {
                    ...
                    if (params.__newObjectClass == "wekb.ComponentVariantName") {
                        ...
                        if (existing_variants) {
                            ...
                            errors.add(g.message(code: 'variantName.value.notUnique', default: 'This variant is already present in this list'))
                        } else {
                    ...
                    if (errors.size() == 0) {
                            ...
                            if (params[p.name] && p.name != 'format') {
                                if (p instanceof Association) {
                                    if (p instanceof ManyToOne || p instanceof OneToOne) {
                                        ...
                                        new_obj[p.name] = genericOIDService.resolveOID3(params[p.name])
                                    } else {
                                        // Add to collection
                                        log.debug("add to collection ${p.name} for OID ${params[p.name]}")
                                        new_obj[p.name].add(genericOIDService.resolveOID3(params[p.name]))
                                    }
                                } else {
                        ...
                        if (params.__refdataName && params.__refdataValue) {
                            ...
                            def refdata = genericOIDService.resolveOID3(params.__refdataValue)
            ...
            if (!domain_class) {
                ...
                flash.error = g.message(code: 'component.classNotFound.label', args: [params.__newObjectClass])
            } else {
                flash.error = g.message(code: 'component.create.denied.label', args: [params.__newObjectClass])
    ...
    def addToStdCollection() {
        ...
        def relatedObj = genericOIDService.resolveOID3(params.__relatedObject)
        ...
        if (relatedObj != null && contextObj != null) {
            ...
            if (editable || user.id == contextObj.id) {
                ...
                flash.error = g.message(code: 'component.list.add.denied.label')
    ...
    def unlinkManyToMany() {
        ...
        if (contextObj) {
            ...
            if (editable || contextObj.id == user.id) {
                def item_to_remove = genericOIDService.resolveOID3(params.__itemToRemove)
                if (item_to_remove) {
                    log.debug("${params}")
                ...
                flash.error = g.message(code: 'component.list.remove.denied.label')
                log.debug("Located instance of context class with oid ${params.__context} is not editable.")
            }
        } else {
            flash.error = g.message(code: 'component.context.notFound.label')
    ...
    def unlinkManyToOne() {
        ...
        if (contextObj) {
            ...
            if (editable) {
                ...
                flash.error = g.message(code: 'component.delete.denied.label')
                log.debug("Located instance of context class with oid ${params.__context} is not editable.")
            }
        } else {
            flash.error = g.message(code: 'component.context.notFound.label')
    ...
    def delete() {
        ...
        if (contextObj) {
            ...
            flash.error = g.message(code: 'component.notFound.label', args: [params.__context])
            log.debug("Unable to locate instance of context class with oid ${params.__context}")
        }
    ...
    def genericSetRel() {
        ...
        if (target != null) {
            def editable = accessService.checkEditableObject(target, params)
            ...
            if ((ns != null) && (owner != null)) {
                ...
                if (editable) {
                    ...
                    try {
                        ...
                        log.debug("${ve}")
                        flash.error = g.message(code: 'identifier.value.illegalIdForm')
                    }
                } else {
                    flash.error = g.message(code: 'component.addToList.denied.label')
                }
            } else {
                flash.error = g.message(code: 'identifier.create.error')
        ...
        if (variant != null) {
            ...
            if (editable) {
                ...
                if (owner.validate()) {
                    ...
                    flash.error = g.message(code: 'variantName.authorize.notUnique')
                }
            } else {
                result.message = "No permission to edit variants for this object!"
                flash.error = g.message(code: 'variantName.owner.denied')
            }
        } else if (!variant) {
            result.message = "Variant with id ${params.id} not found!".toString()
            def vname = g.message(code: 'variantName.label')
            flash.message = g.message(code: 'default.not.found.message', args: [vname, params.id])
        }
    ...
    def deleteVariant() {
        ...
        if (variant != null) {
            def editable = accessService.checkEditableObject(variantOwner, params)
            ...
            def vname = g.message(code: 'variantName.label')
            flash.error = g.message(code: 'default.not.found.message', args: [vname, params.id])
            result.message = "Variant with id ${params.id} not found!".toString()
        }
    ...
    def deleteCoverageStatement() {
        ...
        if (tcs != null) {
            def editable = accessService.checkEditableObject(tipp, params)

            if (editable) {
                tcs.delete()
            } else {
                result.message = "This TIPP is not editable!"
                flash.error = g.message(code: 'tipp.coverage.denied.label')
            }
        } else if (!tcs) {
            def vname = g.message(code: 'TIPPCoverageStatement.label')
    ...
    def deletePrice() {
        ...
        if (c) {
            def editable = accessService.checkEditableObject(c.tipp, params)
            ...
            if (relatedObj) {
                ...
                if (editable) {
                            ...
                            if (!packageVendor) {
                                ...
                                flash.error = g.message(code: 'notUnique.value')
                            }
                } else {
                    flash.error = g.message(code: 'default.noPermissons')
                }
            } else {
                flash.error = g.message(code: 'component.notFound.forLink')
            }
        } else {
            fail = true
        }
    ...
    def addUserToCuratoryGroup() {
        ...
        User contextObj = genericOIDService.resolveOID3(params.__user)
        def user = springSecurityService.currentUser
        CuratoryGroup curatoryGroup = genericOIDService.resolveOID3(params.__curatoryGroup)
    ...
    def addRoleToUser() {
        ...
        User contextObj = genericOIDService.resolveOID3(params.__user)
        def user = springSecurityService.currentUser
        Role role = genericOIDService.resolveOID3(params.__role)
