<%@page import="wekb.CuratoryGroup; wekb.TitleInstancePackagePlatform;"%>
<g:set var="curatoryGroups" value="${(d instanceof TitleInstancePackagePlatform && d.pkg) ? d.pkg.curatoryGroups : d.curatoryGroups }" />

<g:set var="cur_editable" value="${ (editable && linkToCur) || ((editable && d.class.name != 'wekb.auth.User' ) && ((curatoryGroups?.size() == 0) || (request.curator?.size() > 0) || (params.curationOverride == "true" && request.user.isAdmin()))) }" />
            ...
            <input type="hidden" name="__property" value="curatoryGroups"/>
              <div class="input-group" style="width:100%;">
               ...
               %{-- <sec:ifAnyGranted roles="ROLE_SUPERUSER">
                  <p>
                  <g:link controller="create" params="${["tmpl": "wekb.CuratoryGroup"]}">New Curatory Group</g:link>
                  </p>
                </sec:ifAnyGranted>--}%
          </td>
        </g:form>
