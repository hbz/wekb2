	...
	<s2ui:tabs elementId='tabs' height='375' data='${tabData}'>
		...
		<g:each var='entry' in='${org.gokb.cred.Role.findAll()}'>
			...
			<g:if test="${su || ( roleName != 'ROLE_SUPERUSER' && !user.hasRole('ROLE_SUPERUSER') )}">
                    <div>
                            <g:checkBox name='${roleName}' checked="${user.hasRole(roleName) ? "true": "false"}"/>
                            <g:if test="${su}">
                              <g:link controller='role' action='edit' id='${entry.id}'>${roleName}</g:link>
                            </g:if>
                            <g:else>
                              ${roleName}
                            </g:else>
            ...
            <g:else>
                    <div>
                            <g:checkBox name='${roleName}' checked="${user.hasRole(roleName) ? "true": "false"}" disabled="true"/>
                            <g:if test="${su}">
                              <g:link controller='role' action='edit' id='${entry.id}'>${roleName}</g:link>
                            </g:if>
                            <g:else>
